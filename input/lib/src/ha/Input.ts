...
namespace ha.input {
	...
	export class Input {
		...
		static pos = (cx: number, cy: number, buffer: HTMLCanvasElement) => {
			let rect: DOMRect = buffer.getBoundingClientRect();

			let canvasScaleX = parseInt(window.getComputedStyle(buffer).width) / buffer.width;
			let canvasScaleY = parseInt(window.getComputedStyle(buffer).height) / buffer.height;
